...
import { ComplaintAddModal } from '@/components/ComplaintAddModal';
import { ComplaintDetailModal } from '@/components/ComplaintDetailModal';
import { ComplaintsTableView } from '@/components/ComplaintsTableView';
import { ComplaintsKanbanView } from '@/components/ComplaintsKanbanView';
...
export const ComplaintsView: React.FC<Props> = ({ activeCompany: company, user }) => {
  ...
  if (loading) return <div className="flex flex-col items-center justify-center py-24"><Loader2 className="animate-spin text-rose-600 mb-4" size={32} /><p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Sinkronisasi Complaints...</p></div>;
    ...
    <div className="flex flex-col gap-6 h-full text-gray-900">
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setIsAddModalOpen(true)} className="px-6 py-3.5 bg-rose-600 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-rose-100 hover:bg-rose-700 transition-all active:scale-95"><Plus size={12} /> Complaint Baru</button>
        </div>
      </div>
      ...
      <Modal isOpen={confirmDelete.isOpen} onClose={() => setConfirmDelete({ isOpen: false, id: null, name: '' })} title="Hapus Keluhan" size="sm">
        <div className="flex flex-col items-center py-6 text-center">
          ...
          <p className="text-sm text-gray-500 font-medium leading-relaxed mt-2 mb-8">Tindakan ini permanen. Seluruh riwayat aktivitas pada keluhan ini akan hilang.</p>
