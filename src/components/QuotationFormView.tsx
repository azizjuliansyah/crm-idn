  ...
  return (
    <div className="bg-[#F9FAFB] min-h-screen pb-24 animate-in fade-in duration-500 font-inter relative">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
           <div className="flex items-center gap-5">
              <button 
                ...
                className="p-2 text-gray-400 hover:text-gray-900 border border-gray-100 rounded-lg hover:bg-gray-50 transition-all"
              >
                <ArrowLeft size={20} />
              </button>
              <div>
                <h1 className="text-xl font-bold text-gray-900 tracking-tight">{editingId ? 'Ubah Penawaran' : 'Penawaran Baru'}</h1>
                <p className="text-[11px] font-medium text-blue-600 uppercase tracking-widest mt-0.5">{quotationNumber}</p>
              </div>
           </div>
           <div className="flex items-center gap-3">
              <button onClick={() => router.push('/dashboard/sales/quotations')} className="px-5 py-2.5 bg-white border border-gray-200 rounded-lg font-semibold text-sm text-gray-600 hover:bg-gray-50 transition-all">Batal</button>
              {editingId && (
                <button 
              ...
              <button onClick={handleSave} disabled={loading} className="px-5 py-2.5 bg-[#4F46E5] text-white rounded-lg font-bold text-sm flex items-center gap-2 hover:bg-[#4338CA] transition-all shadow-md disabled:opacity-50">
      ...
      <div className="max-w-7xl mx-auto px-10 py-8 space-y-6">
         <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8">
            <div className="flex items-center gap-3 mb-8">
               ...
               <h2 className="text-base font-bold text-gray-800">Rincian Penawaran</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
               <div className="space-y-1.5 relative" ref={clientDropdownRef}>
                  ...
                  <button onClick={() => setIsClientSearchOpen(!isClientSearchOpen)} className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-lg font-medium text-sm text-left flex justify-between items-center group relative shadow-sm transition-all focus:border-blue-500">
               ...
               <div className="space-y-1.5"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Nomor Penawaran</label><input type="text" value={quotationNumber} onChange={e => setQuotationNumber(e.target.value)} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg font-medium text-sm shadow-sm" /></div>
               <div className="space-y-1.5"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Status</label><select value={status} onChange={e => setStatus(e.target.value)} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg font-medium text-sm outline-none cursor-pointer shadow-sm"><option value="Draft">Draft</option><option value="Sent">Sent</option><option value="Accepted">Accepted</option><option value="Declined">Declined</option></select></div>
               <div className="space-y-1.5"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Tanggal</label><input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg font-medium text-sm shadow-sm" /></div>
               <div className="space-y-1.5"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Berlaku Sampai</label><input type="date" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg font-medium text-sm shadow-sm" /></div>
               <div className="space-y-1.5"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">Hubungkan Deal (Optional)</label><select value={dealId || ''} onChange={e => setDealId(e.target.value ? Number(e.target.value) : null)} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg font-medium text-sm shadow-sm cursor-pointer"><option value="">-- Tidak Terhubung Deal --</option>{deals.map(d => (<option key={d.id} value={d.id}>#{String(d.id).padStart(4, '0')} - {d.name}</option>))}</select></div>
            </div>
         </div>
         ...
         <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8">
            <div className="flex items-center gap-3 mb-8">
               ...
               <h2 className="text-base font-bold text-gray-800">Item Produk</h2>
            </div>
                  ...
                  {items.map((item, idx) => (
                    <tr key={idx} className="group">
                      <td className="px-8 py-5 relative" ref={idx === activeRowIdx ? productDropdownRef : null}>
                         ...
                         {activeRowIdx === idx && (
                           <div className="absolute top-[calc(100%-8px)] left-8 right-8 bg-white border border-gray-100 shadow-xl z-50 rounded-lg overflow-hidden animate-in zoom-in-95">
                      ...
                      <td className="px-4 py-5 text-right font-bold text-gray-700 text-sm">Rp {formatIDRVal(item.total)}</td>
            ...
            <button onClick={handleAddItem} className="mt-8 flex items-center gap-2 px-4 py-2 text-[#4F46E5] font-bold text-[11px] uppercase tracking-[0.15em] hover:bg-indigo-50 rounded-lg transition-all">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 space-y-4">
               <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Catatan</label>
               <textarea value={notes} onChange={e => setNotes(e.target.value)} className="w-full px-6 py-5 bg-white border border-gray-100 rounded-xl h-44 outline-none resize-none focus:bg-white focus:border-blue-500 transition-all font-medium text-xs text-gray-700 shadow-sm" placeholder="Tambahkan catatan untuk pelanggan..." />
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 space-y-5">
               ...
               <div className="flex items-center justify-between border-b border-gray-50 pb-4">
                  <div className="flex items-center gap-4">
                     ...
                     <div className="flex bg-white rounded border border-gray-200 overflow-hidden shadow-sm h-9">
                        <select value={discountType} onChange={e => setDiscountType(e.target.value as any)} className="bg-gray-50 px-2 border-r text-[10px] font-bold uppercase outline-none cursor-pointer">
               ...
               <div className="pt-2 pb-6 flex items-center justify-between">
                  ...
                  <span className="text-2xl font-bold text-[#4F46E5]">Rp {formatIDRVal(total)}</span>
               </div>
        ...
        <div className="space-y-8 py-2">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="col-span-2 space-y-2">
                 ...
                 <input type="text" value={quickClientForm.name} onChange={e => setQuickClientForm({...quickClientForm, name: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 border border-gray-100 rounded-xl font-bold text-sm outline-none" placeholder="Nama pelanggan..." />
              </div>
              <div className="space-y-2">
                 <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Email</label>
                 <input type="email" value={quickClientForm.email} onChange={e => setQuickClientForm({...quickClientForm, email: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 border border-gray-100 rounded-xl font-bold text-sm outline-none" placeholder="Email..." />
              </div>
              <div className="space-y-2">
                 <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">WhatsApp</label>
                 <input type="text" value={quickClientForm.whatsapp} onChange={e => setQuickClientForm({...quickClientForm, whatsapp: e.target.value})} className="w-full px-5 py-3.5 bg-gray-50 border border-gray-100 rounded-xl font-bold text-sm outline-none" placeholder="08..." />
              </div>
           </div>
           
           <div className="space-y-3 pt-4 border-t border-gray-50">
              <div className="flex items-center justify-between">
                 <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Perusahaan (Opsional)</label>
                 <button onClick={() => setIsAddingCo(true)} className="text-[10px] font-bold text-blue-600 uppercase tracking-widest hover:underline">+ Daftar Baru</button>
              </div>
              {!isAddingCo ? (
                <select value={quickClientForm.client_company_id} onChange={e => setQuickClientForm({...quickClientForm, client_company_id: e.target.value})} className="w-full px-4 py-3.5 bg-gray-50 border border-gray-100 rounded-xl font-bold text-xs">
                ...
                <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 space-y-3">
                   <div className="flex justify-between items-center"><span className="text-[10px] font-bold text-blue-600 uppercase">Tambah Perusahaan</span><button onClick={() => setIsAddingCo(false)} className="text-gray-400"><X size={14} /></button></div>
                   <input type="text" placeholder="Nama Perusahaan" value={newCo.name} onChange={e => setNewCo({...newCo, name: e.target.value})} className="w-full px-4 py-2 text-xs border rounded-lg" />
                   <div className="flex gap-2">
                     <select value={newCo.category_id} onChange={e => setNewCo({...newCo, category_id: e.target.value})} className="flex-1 px-4 py-2 text-xs border rounded-lg"><option value="">Kategori...</option>{clientCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                     <button onClick={handleQuickAddCo} disabled={coProcessing} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold">Simpan</button>
                   </div>
                </div>
              )}
           </div>
      ...
      <Modal
        ...
        size="lg"
        footer={<button onClick={handleQuickAddProduct} disabled={isProcessingQuick} className="px-10 py-4 bg-emerald-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-xl flex items-center gap-2 active:scale-95">{isProcessingQuick && <Loader2 className="animate-spin" size={14} />} Simpan Produk</button>}
      >
        <div className="space-y-6 py-2">
           <div className="space-y-2">
             <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Nama Produk</label>
             <input type="text" value={quickProductForm.name} onChange={e => setQuickProductForm({...quickProductForm, name: e.target.value})} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-xl font-bold text-sm outline-none" placeholder="Nama produk..." />
           </div>
           
           <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                 <div className="flex justify-between"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Kategori</label><button onClick={() => setIsAddingCat(!isAddingCat)} className="text-[10px] text-blue-600 font-bold uppercase hover:underline">+ Baru</button></div>
                 {!isAddingCat ? (
                   <select value={quickProductForm.category_id} onChange={e => setQuickProductForm({...quickProductForm, category_id: e.target.value})} className="w-full px-4 py-3.5 bg-gray-50 border border-gray-100 rounded-xl font-bold text-xs"><option value="">Pilih</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                 ) : (
                   <div className="flex gap-2"><input type="text" value={newCatName} onChange={e => setNewCatName(e.target.value)} className="flex-1 px-3 py-2 text-xs border rounded-lg" placeholder="Nama Kategori" /><button onClick={handleQuickAddCat} className="px-3 bg-blue-600 text-white rounded-lg text-xs"><CheckIcon size={14} /></button></div>
                 )}
              </div>
              <div className="space-y-2">
                 <div className="flex justify-between"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Satuan</label><button onClick={() => setIsAddingUnit(!isAddingUnit)} className="text-[10px] text-blue-600 font-bold uppercase hover:underline">+ Baru</button></div>
                 {!isAddingUnit ? (
                   <select value={quickProductForm.unit_id} onChange={e => setQuickProductForm({...quickProductForm, unit_id: e.target.value})} className="w-full px-4 py-3.5 bg-gray-50 border border-gray-100 rounded-xl font-bold text-xs"><option value="">Pilih</option>{units.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select>
                 ) : (
                   <div className="flex gap-2"><input type="text" value={newUnitName} onChange={e => setNewUnitName(e.target.value)} className="flex-1 px-3 py-2 text-xs border rounded-lg" placeholder="Nama Unit" /><button onClick={handleQuickAddUnit} className="px-3 bg-blue-600 text-white rounded-lg text-xs"><CheckIcon size={14} /></button></div>
                 )}
              </div>
           </div>

           <div className="space-y-2">
             <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Harga Satuan (Rp)</label>
             <input type="number" value={quickProductForm.price} onChange={e => setQuickProductForm({...quickProductForm, price: Number(e.target.value)})} className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-xl font-bold text-lg text-blue-600 outline-none" />
           </div>
        </div>
      </Modal>
