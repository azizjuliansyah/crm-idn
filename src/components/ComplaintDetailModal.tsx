          ...
          <div className="w-1/2 flex flex-col space-y-6 overflow-y-auto custom-scrollbar pr-2">
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                   ...
                   <div className="space-y-1.5"><label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider ml-0.5">PIC Penanggung Jawab</label><select value={form.assigned_id || ''} onChange={e => setForm({...form, assigned_id: e.target.value})} className="w-full h-11 bg-gray-50 border border-gray-100 rounded-sm px-3 text-xs font-bold outline-none focus:bg-white focus:border-rose-400 transition-all cursor-pointer">{members.map(m => <option key={m.profile?.id} value={m.profile?.id}>{m.profile?.full_name}</option>)}</select></div>
                </div>
