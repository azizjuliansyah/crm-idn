      ...
      <div className="flex flex-col h-[88vh] bg-white text-[#111827] overflow-hidden rounded-md border border-gray-200">
        <header className="px-6 h-16 flex items-center justify-between border-b border-gray-100 bg-white shrink-0">
           <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <h2 className="text-base font-bold tracking-tight text-gray-900 uppercase">{form.title}</h2>
              </div>
           </div>
           <div className="flex items-center gap-3">
              ...
              <button onClick={handleSave} disabled={isProcessing} className="h-10 px-6 bg-gray-900 text-white rounded font-bold text-xs uppercase tracking-wider flex items-center gap-2 hover:bg-black transition-all">
                 {isProcessing ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />} UPDATE COMPLAINT
              </button>
        ...
        <div className="px-8 py-4 flex items-center justify-between border-b border-gray-50 bg-gray-50/30 shrink-0">
           <div className="flex items-center gap-6">
              <div className="px-2 py-0.5 border border-gray-200 rounded-sm text-[10px] font-bold text-gray-500 bg-white uppercase tracking-widest">ID: {ticket.id}</div>
              <div className="flex items-center gap-1.5 text-[11px] font-medium text-gray-400"><Clock size={14} /> Terdaftar {new Date(ticket.created_at).toLocaleDateString('id-ID', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
           </div>
           <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Prioritas:</span>
              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${form.priority === 'urgent' ? 'bg-rose-100 text-rose-600' : 'bg-orange-100 text-orange-600'}`}>{form.priority}</span>
           </div>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden px-8 py-6 gap-10">
          <div className="w-1/2 flex flex-col border-r border-gray-100 pr-10">
             <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-6">Penanganan & Resolusi</h3>
             <div className="flex-1 flex flex-col overflow-hidden">
                <div className="flex items-start gap-4 mb-8">
                   <div className="w-9 h-9 rounded bg-rose-600 text-white flex items-center justify-center font-bold text-[10px] shrink-0 shadow-md shadow-rose-100">{user.full_name.charAt(0)}</div>
                   <div className="flex-1 relative">
                      <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Tulis langkah penyelesaian..." className="w-full min-h-[80px] bg-gray-50 border border-gray-200 rounded-sm p-3 text-xs font-medium outline-none focus:bg-white focus:border-rose-400 transition-all resize-none" />
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-6">
                   {loadingActivities ? <div className="py-20 text-center animate-pulse text-[10px] uppercase font-bold text-gray-300">Mensinkronisasi Aktivitas...</div> : activities.length === 0 ? <div className="py-20 text-center text-gray-300 italic text-[10px] uppercase tracking-widest">Belum ada progres tercatat</div> : activities.map((act, i) => (
          ...
          <div className="w-1/2 flex flex-col space-y-6 overflow-y-auto custom-scrollbar pr-2">
             <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Data Pelaporan</h3>
             <div className="space-y-4">
                <div className="space-y-1.5"><label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider ml-0.5">Subjek / Masalah</label><input type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full h-11 bg-gray-50 border border-gray-100 rounded-sm px-4 text-xs font-bold outline-none focus:bg-white focus:border-rose-400 transition-all shadow-inner" /></div>
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1.5"><label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider ml-0.5">Pelapor (Client)</label><select value={form.client_id || ''} onChange={e => setForm({...form, client_id: Number(e.target.value)})} className="w-full h-11 bg-gray-50 border border-gray-100 rounded-sm px-3 text-xs font-bold outline-none focus:bg-white focus:border-rose-400 transition-all cursor-pointer">{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                   <div className="space-y-1.5"><label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider ml-0.5">PIC Penanggung Jawab</label><select value={form.assigned_id || ''} onChange={e => setForm({...form, assigned_id: e.target.value})} className="w-full h-11 bg-gray-50 border border-gray-100 rounded-sm px-3 text-xs font-bold outline-none focus:bg-white focus:border-rose-400 transition-all cursor-pointer">{members.map(m => <option key={m.profiles?.id} value={m.profiles?.id}>{m.profiles?.full_name}</option>)}</select></div>
                </div>
                <div className="space-y-1.5"><label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider ml-0.5">Kronologi Kejadian</label><textarea value={form.description || ''} onChange={e => setForm({...form, description: e.target.value})} className="w-full min-h-[140px] bg-gray-50 border border-gray-100 rounded-sm p-4 text-xs font-medium outline-none focus:bg-white focus:border-rose-400 transition-all resize-none shadow-inner" /></div>
             </div>
          </div>
        </div>
      </div>
