    ...
    <div className="flex flex-col gap-6 h-full text-gray-900">
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex flex-wrap items-center gap-3 flex-1">
          ...
          <div className="flex items-center gap-2">
            <select 
              ...
              className="px-3 py-2.5 bg-white border border-gray-100 rounded-xl text-[10px] font-bold uppercase tracking-widest text-gray-500 outline-none shadow-sm min-w-[150px]"
            >
        ...
        <button 
          ...
          className="px-6 py-3.5 bg-indigo-600 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
        >
          <Plus size={14} /> Buat Proforma
        </button>
      </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredProformas.map(p => (
                <tr key={p.id} className="hover:bg-gray-50/30 group transition-colors">
                  <td className="px-8 py-6 text-xs font-bold text-gray-500">{formatDateString(p.date)}</td>
                  <td className="px-8 py-6">
                     <button onClick={() => router.push(`/dashboard/sales/proformas/${p.id}`)} className="font-bold text-indigo-600 text-xs tracking-tight hover:underline">{p.number}</button>
                  </td>
                  <td className="px-8 py-6">
                    <p className="text-xs font-bold text-gray-900">{p.client?.name || 'Umum'}</p>
                    <p className="text-[9px] text-gray-400 font-bold uppercase mt-1">{p.client?.client_company?.name || 'Personal'}</p>
                  </td>
                  <td className="px-8 py-6 text-right font-bold text-indigo-600 text-xs">{formatIDR(p.total)}</td>
                  <td className="px-8 py-6 text-center">
                    <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleDownloadPDF(p)} className="p-2 text-emerald-500 hover:bg-emerald-50 rounded-lg"><FileDown size={14} /></button>
                      <button onClick={() => router.push(`/dashboard/sales/proformas/${p.id}`)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><Edit2 size={14} /></button>
                      <button onClick={() => setConfirmDelete({ isOpen: true, id: p.id, number: p.number })} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {filteredProformas.length === 0 && (
                <tr><td colSpan={5} className="py-24 text-center opacity-30 italic"><FileCheck size={48} className="mx-auto mb-4" /><p className="text-xs font-bold uppercase tracking-widest">Belum ada proforma tercatat</p></td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal isOpen={confirmDelete.isOpen} onClose={() => setConfirmDelete({ isOpen: false, id: null, number: '' })} title="Hapus Proforma" size="sm">
