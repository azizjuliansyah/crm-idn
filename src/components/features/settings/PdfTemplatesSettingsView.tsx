...
export const PdfTemplatesSettingsView: React.FC<Props> = ({ company }) => {
  ...
  if (loading) return <div className="flex flex-col items-center justify-center py-24"><Loader2 className="animate-spin text-blue-600 mb-4" /><p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Memuat Pengaturan Template...</p></div>;
    ...
    <div className="space-y-8 max-w-4xl">
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
        <div className="p-8 border-b border-gray-50">
          <h3 className="text-lg font-bold text-gray-900 tracking-tight">Desain Dokumen PDF</h3>
          <p className="text-xs text-gray-400 font-medium mt-1">Pilih tata letak dan desain untuk dokumen bisnis Anda.</p>
        </div>
         ...
         <div className="flex border-b border-gray-100">
           {DOCUMENT_TYPES.map(type => (
             <Button
               ...
               className={`flex-1 !py-4 !text-xs font-bold uppercase tracking-widest transition-all !rounded-none border-b-2 shadow-none ${
                 activeTab === type.id 
              ...
              {TEMPLATE_OPTIONS.map(template => {
                ...
                return (
                  ...
                  >
                    ...
                    <h4 className={`text-sm font-bold ${isActive ? 'text-blue-700' : 'text-gray-900'}`}>{template.name}</h4>
