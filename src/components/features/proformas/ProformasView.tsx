            ...
            <TableBody>
              {filteredProformas.map(p => (
                <TableRow key={p.id} className="group hover:bg-indigo-50/30 transition-colors border-b border-gray-50/50 last:border-0">
                  <TableCell className="text-[11px] text-gray-500 py-5 px-6">{formatDateString(p.date)}</TableCell>
                  <TableCell className="py-5 px-6">
                    <Button onClick={() => router.push(`/dashboard/sales/proformas/${p.id}`)} className=" text-indigo-600 text-xs tracking-tight hover:underline flex items-center gap-1.5">
                  ...
                  <TableCell className="py-5 px-6">
                    <div className="flex flex-col">
                      <Label className="text-xs text-gray-900 tracking-tight">{p.client?.name || 'Umum'}</Label>
                      <Label className="text-[10px] !text-gray-400 mt-1 uppercase tracking-tight italic">{p.client?.client_company?.name || 'Personal'}</Label>
                    </div>
                  </TableCell>
                  <TableCell className="text-right  text-indigo-600 text-xs py-5 px-6 bg-indigo-50/5 group-hover:bg-indigo-50/20">{formatIDR(p.total)}</TableCell>
                  <TableCell>
