               ...
               return (
                  <div key={item.id} className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${isSelected ? 'bg-blue-50/50 border-blue-200' : 'bg-white border-gray-100'}`}>
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           <p className="text-[13px] font-bold text-gray-900">{accessMode.type === 'user_to_companies' ? item.name : item.full_name}</p>
                           <p className="text-[10px] text-gray-400 font-bold uppercase">{accessMode.type === 'user_to_companies' ? `ID: ${item.id}` : item.email}</p>
                        </div>
                     </div>
                     {isSelected && roles.length > 0 && (
                        <Select 
                           value={selectedWithRoles[item.id] || roles[0]?.id} 
                           onChange={e => onToggleAccessWithRole(item.id, e.target.value)}
                           className="py-1.5 text-[10px] text-blue-600 border-blue-100 w-auto"
                        >
