...
import { Modal, Button, Input, Select, Textarea, Badge, Timeline, TimelineItem, TimelineIcon, TimelineContent, H2, Subtext } from '@/components/ui';
      ...
      <div className="flex flex-col h-[90vh] bg-white text-gray-900 overflow-hidden rounded-2xl border border-gray-100 shadow-2xl">
        <header className="px-6 h-16 flex items-center justify-between border-b border-gray-100 bg-white shrink-0">
           <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <H2 className="text-base tracking-tight font-black uppercase">{form.title}</H2>
              </div>
           </div>
           <div className="flex items-center gap-3">
              ...
              <Button onClick={handleSave} disabled={isProcessing} isLoading={isProcessing} leftIcon={<Save size={14} />} variant="danger" className="h-10 px-6 font-bold text-xs uppercase tracking-[0.1em] shadow-lg shadow-rose-100">
                 UPDATE TICKET
              </Button>
        ...
        <div className="px-8 py-4 flex items-center justify-between border-b border-gray-100 bg-gray-50/50 shrink-0">
           <div className="flex items-center gap-6">
              <Badge variant="ghost" className="px-2.5 py-1 border border-gray-200 text-[9px] font-black text-gray-600 bg-white uppercase tracking-tighter ring-1 ring-inset ring-gray-100">ID TICKET: {ticket.id}</Badge>
              <div className="flex items-center gap-1.5 text-[10px] font-black text-gray-400 uppercase tracking-widest"><Clock size={14} className="text-gray-300" /> Dibuat {new Date(ticket.created_at).toLocaleDateString('id-ID', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
              <Badge variant={form.type === 'complaint' ? 'danger' : 'primary'} className="flex items-center gap-1.5 px-3 py-1 text-[9px] font-black uppercase tracking-[0.2em] border-none ring-1 ring-inset shadow-sm">
                <Layers size={10} /> {form.type === 'complaint' ? 'COMPLAINT' : 'STANDARD TICKET'}
              </Badge>
           </div>
           <div className="flex items-center gap-2">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Urgensi:</span>
              <Badge variant={form.priority === 'urgent' ? 'danger' : 'secondary'} className="text-[10px] py-1 px-3 font-black uppercase tracking-widest ring-1 ring-inset">{form.priority}</Badge>
           </div>
        </div>
        ...
        <div className="px-8 pt-6 pb-2 shrink-0">
           <div className="flex items-center h-11 w-full overflow-hidden rounded-xl border border-gray-200 p-1 bg-gray-50/50">
              {stages.map((stage, idx) => {
                ...
                return (
                  <button key={stage.id} onClick={() => handleStatusChange(stage.name)} className={`relative flex-1 h-full flex items-center justify-center transition-all text-[9px] font-black uppercase tracking-[0.15em] rounded-lg ${bgColor}`}>
                    {stage.name}
                  </button>
                );
              })}
        ...
        <div className="flex-1 flex overflow-hidden px-8 py-6 gap-10">
          <div className="w-1/2 flex flex-col border-r border-gray-100 pr-10">
             <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.25em] mb-6 flex items-center gap-2"><MessageSquare size={12} className="text-gray-300" /> Log Percakapan & Status</h3>
             <div className="flex-1 flex flex-col overflow-hidden">
                <div className="flex items-start gap-4 mb-8">
                   <div className="w-10 h-10 rounded-xl bg-rose-600 text-white flex items-center justify-center font-bold text-[12px] shrink-0 uppercase shadow-lg shadow-rose-100">{user.full_name?.charAt(0)}</div>
                   ...
                   <Timeline>
                      {loadingActivities ? (
                        <div className="py-20 flex flex-col items-center gap-3">
                           ...
                           <span className="text-[9px] font-black uppercase text-gray-300 tracking-[0.2em]">Memuat Log...</span>
                        </div>
                      ) : activities.length === 0 ? (
                        <div className="py-20 text-center text-gray-200 italic text-[9px] font-black uppercase tracking-[0.3em]">Belum Ada Aktivitas</div>
                      ) : activities.map((act) => (
                        <TimelineItem key={act.id} className="pb-8">
                          ...
                          <TimelineContent>
                            <div className="flex items-center gap-3 mb-1.5">
                              <span className="text-[11px] font-black text-gray-900 tracking-tight">{act.profile?.full_name}</span>
                              <span className="text-[8px] font-black text-gray-300 uppercase tracking-widest bg-gray-50 px-1.5 py-0.5 rounded">{new Date(act.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} • {new Date(act.created_at).toLocaleDateString('id-ID', { month: 'short', day: 'numeric' })}</span>
                            </div>
                            <p className={`text-[12px] font-medium leading-relaxed ${act.activity_type === 'status_change' ? 'text-gray-400 italic font-bold' : 'text-gray-600'}`}>{act.content}</p>
                          </TimelineContent>
                        </TimelineItem>
                      ))}
                   </Timeline>
          ...
          <div className="w-1/2 flex flex-col space-y-8 overflow-y-auto scrollbar-hide pr-2">
             <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.25em] flex items-center gap-2"><Layers size={12} className="text-gray-300" /> Konfigurasi Data Tiket</h3>
