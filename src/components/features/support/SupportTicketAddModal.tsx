...
export const SupportTicketAddModal: React.FC<Props> = ({ 
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess();
      onClose();
      setForm({ title: '', description: '', client_id: null, assigned_id: members[0]?.user_id || '', status: stages[0]?.name.toLowerCase() || 'open', priority: 'normal', type: 'ticket' });
    } catch (err: any) {
      alert(err.message);
    } finally {
    ...
    <Modal 
      ...
      footer={
        <div className="flex w-full gap-3">
          <Button variant="ghost" onClick={onClose} className="flex-1 font-bold text-xs uppercase tracking-widest">Batal</Button>
          <Button 
            onClick={handleSave} 
