        ...
        return (
          <div key={stage.id} onDragOver={(e) => onDragOver(e, sKey)} onDrop={(e) => onDrop(e, sKey)} className="flex flex-col gap-3 min-w-[280px] w-[280px] h-full group/column">
            <div className={`p-4 ${COLORS[idx % COLORS.length]} rounded-2xl shadow-lg shadow-gray-200/50 flex items-center justify-between text-white transition-all`}>
              <Label className="text-[10px]  uppercase tracking-[0.2em]">{stage.name}</Label>
              <Label className="text-[10px]  bg-white/20 px-2.5 py-0.5 rounded-full ring-1 ring-white/30">{ticketsByStatus[sKey]?.length || 0}</Label>
            </div>
            <div className={`flex-1 space-y-3 p-2 rounded-2xl border-2 border-dashed transition-all overflow-y-auto custom-scrollbar ${dropTarget === sKey ? 'bg-rose-50 border-rose-300' : 'bg-gray-50/50 border-gray-100'}`}>
              {ticketsByStatus[sKey]?.map(t => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <Subtext className="text-[9px] font-extrabold text-gray-400 font-mono tracking-tight">#{String(t.id).padStart(4, '0')}</Subtext>
                    <div className="flex items-center gap-1.5">
                      <Badge variant={t.type === 'complaint' ? 'danger' : 'primary'} className="text-[7px] py-0.5 px-2 border-none ring-1 ring-inset tracking-tight uppercase">
                        {t.type || 'ticket'}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-gray-100 flex items-center justify-between text-gray-400 text-[9px] ">
                    <div className="flex items-center gap-2">
                      ...
                      <Label className="tracking-tight text-gray-500">{t.assigned_profile?.full_name?.split(' ')[0]}</Label>
                    </div>
              ...
              {ticketsByStatus[sKey]?.length === 0 && (
                <div className="py-12 text-center opacity-10 flex flex-col items-center grayscale">
                  ...
                  <Label className="text-[8px]  uppercase tracking-tight">Kosong</Label>
                </div>
              )}
            </div>
          </div>
