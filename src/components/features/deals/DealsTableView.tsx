...
import { Button, Table, TableHeader, TableBody, TableRow, TableCell, TableEmpty, Label, Badge, InfiniteScrollSentinel } from '@/components/ui';
        ...
        <Table>
          ...
          <TableBody>
                    ...
                    <div className="mt-1">
                      {quotation ? (
                        ...
                        onCreateQuotation && d.client_id && (
                          <Link
                            href={`/dashboard/sales/quotations/create?client_id=${d.client_id}&deal_id=${d.id}`}
                            className="inline-flex items-center gap-1 px-2 py-1 text-gray-500 hover:text-indigo-600 text-[10px] font-medium transition-colors"
                          >
            ...
            {deals.length === 0 && !isLoadingMore && (
              <TableEmpty colSpan={8} message="Tidak ada transaksi ditemukan" />
            )}
          </TableBody>
        </Table>
      </div>
