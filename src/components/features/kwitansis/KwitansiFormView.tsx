...
import { Company, Client, Product, Kwitansi, KwitansiItem, TaxSetting, ProductCategory, ProductUnit, ClientCompany, ClientCompanyCategory, Quotation, AutonumberSetting } from '@/lib/types';
import {
...
export const KwitansiFormView: React.FC<Props> = ({ company, editingId, kwitansiId, initialClientId, initialProformaId, initialQuotationId, initialInvoiceId, initialRequestId, onSaveSuccess, onBack }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (editingId || kwitansiId) {
      const idToFetch = editingId || Number(kwitansiId);
      const { data: invData } = await supabase.from('kwitansis').select('*, kwitansi_items(*)').eq('id', idToFetch).single();
      if (invData) {
        existing = invData;
        setClientId(String(invData.client_id));
        setProformaId(invData.proforma_id);
        setQuotationId(invData.quotation_id);
        setInvoiceId(invData.invoice_id);
        setKwitansiNumber(invData.number);
        ...
        setItems(invData.kwitansi_items.map((it: any) => ({
          id: it.id,
    ...
    const [cRes, pRes, tRes, nRes, qRes, pfRes, invoicesRes, catRes, unitRes, coRes, coCatRes] = await Promise.all([
      ...
      supabase.from('autonumber_settings').select('*').eq('company_id', company.id).eq('document_type', 'kwitansi').maybeSingle(),
    ...
    if (!editingId) {
      ...
      if (initialProformaId) {
        ...
        if (pfData) {
          ...
          const taxIds = pfData.tax_type ? (tRes.data?.filter(t => pfData.tax_type.includes(t.name)).map(t => t.id) || []) : [];
        ...
        if (qData) {
          ...
          const taxIds = qData.tax_type ? (tRes.data?.filter(t => qData.tax_type.includes(t.name)).map(t => t.id) || []) : [];
          setSelectedTaxIds(taxIds);
        ...
        const { data: invData } = await supabase.from('invoices').select('*, invoice_items(*)').eq('id', initialInvoiceId).single();
        if (invData) {
          setClientId(String(invData.client_id));
          setProformaId(invData.proforma_id);
          setQuotationId(invData.quotation_id);
          ...
          setDiscountType(invData.discount_type as any);
          setDiscountValue(invData.discount_value);
          const taxIds = invData.tax_type ? (tRes.data?.filter(t => invData.tax_type.includes(t.name)).map(t => t.id) || []) : [];
          setSelectedTaxIds(taxIds);
      ...
      if (nRes.data) {
        ...
        const formattedNumber = generateFormattedNumber(nRes.data.format_pattern, actualNextNumber, nRes.data.digit_count || 4, nRes.data.prefix || 'KWT');
        setKwitansiNumber(formattedNumber);
    ...
    setLoading(false);
  }, [company.id, editingId, kwitansiId, initialClientId, initialProformaId, initialQuotationId, initialInvoiceId]);
  ...
  const handleInvoiceChange = async (id: string | number) => {
    const val = id ? Number(id) : null;
    setInvoiceId(val);
    if (val) {
      setLoading(true);
      const { data: invData } = await supabase.from('invoices').select('*, invoice_items(*)').eq('id', val).single();
      if (invData) {
        setClientId(String(invData.client_id));
        setProformaId(invData.proforma_id);
        setQuotationId(invData.quotation_id);
        ...
        setDiscountType(invData.discount_type as any);
        setDiscountValue(invData.discount_value);
        const taxIds = invData.tax_type ? (availableTaxes.filter(t => invData.tax_type.includes(t.name)).map(t => t.id) || []) : [];
        setSelectedTaxIds(taxIds);
        setItems(invData.invoice_items.map((it: any) => ({
          productId: String(it.product_id),
          description: it.description || '',
      ...
      // Reset if reference cleared? Maybe keep previous data or clear it?
      // User said "data yang berdasarkan invoice harus mengikuti referensi nya"
      // Suggests clear if reference cleared.
      setClientId('');
      setProformaId(null);
      setQuotationId(null);
      setStatus('Unpaid');
      setItems([{ productId: '', description: '', qty: 1, unit: 'pcs', price: 0, total: 0 }]);
      const defTaxIds = availableTaxes.filter(t => t.is_default).map(t => t.id) || [];
      setSelectedTaxIds(defTaxIds);
    }
  };
  ...
  const handleSave = async () => {
    if (!clientId || !KwitansiNumber) return;
    setLoading(true);
    try {
      ...
      let invId = editingId;
      if (editingId) {
        await supabase.from('kwitansis').update(payload).eq('id', editingId);
        await supabase.from('kwitansi_items').delete().eq('kwitansi_id', editingId);
      } else {
        const { data: invData, error: invErr } = await supabase.from('kwitansis').insert(payload).select().single();
      ...
      if (itemInserts.length > 0) await supabase.from('kwitansi_items').insert(itemInserts);
      if (!editingId) {
        const { data: nSet } = await supabase.from('autonumber_settings').select('next_number').eq('company_id', company.id).eq('document_type', 'kwitansi').maybeSingle();
        if (nSet) await supabase.from('autonumber_settings').update({ next_number: nSet.next_number + 1 }).eq('company_id', company.id).eq('document_type', 'kwitansi');
      }

      if (invId) {
        if (!editingId && initialRequestId) {
          await supabase.from('kwitansi_requests').update({ kwitansi_id: invId, status: 'Approved' }).eq('id', initialRequestId);
        }
        onSaveSuccess?.(invId);
  ...
  const handleDownloadPDF = async () => {
    if (!editingId && !kwitansiId) return;
    const idToFetch = editingId || Number(kwitansiId);
    ...
    if (templateId === 'template1') {
      ...
      autoTable(doc, {
        ...
        body: inv.kwitansi_items?.map((it: any) => [
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-gray-100 px-10 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-5">
            <Button variant="ghost" onClick={onBack || (() => router.push('/dashboard/sales/kwitansis'))} className="!p-2 text-gray-400 border border-gray-100 h-10 w-10">
              <ArrowLeft size={20} />
            </Button>
            <div>
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" onClick={onBack || (() => router.push('/dashboard/sales/kwitansis'))}>Batal</Button>
            {(editingId || kwitansiId) && (
              <Button
                variant="secondary"
            ...
            <Button onClick={handleSave} isLoading={loading} disabled={!clientId} leftIcon={(editingId || kwitansiId) ? <Save size={16} /> : <CheckCircle2 size={16} />} variant="primary">
              {(editingId || kwitansiId) ? 'Update Kwitansi' : 'Simpan Kwitansi'}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-10 py-8 space-y-6">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
            ...
            <ComboBox
              ...
              value={invoiceId ? invoiceId.toString() : ''}
              onChange={handleInvoiceChange}
              options={[
                { value: '', label: '-- Tanpa Referensi --' },
                ...invoices.map(inv => ({ value: inv.id.toString(), label: inv.number }))
              ]}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <Card className="p-8 space-y-5">
            ...
            >
              {editingId ? 'Update Kwitansi' : 'Simpan Kwitansi'}
            </Button>
          </Card>
        </div>
      </div>
