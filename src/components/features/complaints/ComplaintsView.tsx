...
import {
  Plus, Trello, Table as TableIcon, Loader2 as LoaderIcon, List, LayoutGrid
} from 'lucide-react';
...
export const ComplaintsView: React.FC<Props> = ({ activeCompany: company, user }) => {
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      showToast(`${selectedIds.length} keluhan berhasil dihapus.`, 'success');
      setSelectedIds([]);
  ...
  const handleBulkUpdateStatus = async (status: string) => {
    try {
      ...
      showToast(`${selectedIds.length} status keluhan berhasil diperbarui.`, 'success');
      setSelectedIds([]);
  ...
  const handleExportComplaints = () => {
    ...
    const columns: ExcelColumn[] = [
      ...
      { header: 'Judul Keluhan', key: 'title', width: 30 },
    ...
    exportToExcel(formattedData, columns, 'CRM_Complaints_Report');
    showToast('Data Keluhan berhasil diekspor ke Excel', 'success');
  };
  ...
  return (
    <div className="flex flex-col gap-6 text-gray-900">
      <StandardFilterBar
        ...
        subtitle="Kelola keluhan dan komplain dari pelanggan."
        searchTerm={filters.searchTerm}
        onSearchChange={filters.setSearchTerm}
        onExport={handleExportComplaints}
        searchPlaceholder="Cari keluhan atau pelanggan..."
        viewModes={{
          current: viewMode,
      ...
      <ConfirmBulkDeleteModal
        ...
        title="Hapus Keluhan Masal"
        description={`Apakah Anda yakin ingin menghapus ${selectedIds.length} keluhan yang dipilih? Tindakan ini permanen.`}
