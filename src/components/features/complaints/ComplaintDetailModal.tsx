...
import { 
  ...
  Avatar, Timeline, TimelineItem, TimelineIcon, TimelineContent, H2, Subtext, Label
} from '@/components/ui';
      ...
      <div className="flex flex-col h-[88vh] bg-white text-[#111827] overflow-hidden rounded-md border border-gray-200">
        ...
        <div className="px-8 py-4 flex items-center justify-between border-b border-gray-50 bg-gray-50/30 shrink-0">
           <div className="flex items-center gap-6">
              <Badge variant="ghost" className="text-gray-500 bg-white uppercase tracking-widest px-2 py-0.5 border border-gray-200">ID: {ticket.id}</Badge>
           ...
           <div className="flex items-center gap-2">
              <Subtext className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mr-2">Prioritas:</Subtext>
              <Badge variant={form.priority === 'urgent' ? 'danger' : 'warning'} className="font-bold uppercase">{form.priority}</Badge>
           </div>
        </div>
              ...
              {stages.map((stage, idx) => {
                ...
                return (
                  <Button 
                    ...
                    className={`relative flex-1 h-full flex items-center justify-center transition-all text-[10px] font-bold uppercase tracking-widest ${isActive ? 'shadow-inner' : 'text-gray-400 hover:bg-gray-50'} border-r last:border-r-0`}
        ...
        <div className="flex-1 flex overflow-hidden px-8 py-6 gap-10">
          <div className="w-1/2 flex flex-col border-r border-gray-100 pr-10">
             <Subtext className="!text-[10px] font-bold uppercase tracking-[0.2em] mb-6">Penanganan & Resolusi</Subtext>
             <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-4">
                   {loadingActivities ? <div className="py-20 text-center animate-pulse text-[10px] uppercase font-bold text-gray-300">Mensinkronisasi Aktivitas...</div> : activities.length === 0 ? <div className="py-20 text-center text-gray-300 italic text-[10px] uppercase tracking-widest">Belum ada progres tercatat</div> : (
                     <Timeline>
                        {activities.map((act) => (
                           <TimelineItem key={act.id}>
                              ...
                              <TimelineContent>
                                 <div className="flex items-center gap-2 mb-1">
                                    <Badge variant="ghost" className="!p-0 text-xs font-bold text-gray-900">{act.profile?.full_name}</Badge>
                                    <Badge variant="ghost" className="!p-0 text-[9px] font-bold text-gray-300 uppercase">{new Date(act.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</Badge>
                                 </div>
          ...
          <div className="w-1/2 flex flex-col space-y-6 overflow-y-auto custom-scrollbar pr-2">
             <Subtext className="!text-[10px] font-bold uppercase tracking-[0.2em]">Data Pelaporan</Subtext>
             <div className="space-y-4">
                <Input label="Subjek / Masalah" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="!font-bold" />
