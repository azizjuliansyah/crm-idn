...
import { Modal, Button, Input, Select, Textarea, SectionHeader, Badge, Label, Subtext, Table, TableHeader, TableBody, TableRow, TableCell } from '@/components/ui';
...
export const InvoiceFormView: React.FC<Props> = ({ company, editingId, initialClientId, initialProformaId, initialQuotationId, onSaveSuccess }) => {
  ...
  if (loading && !items.length) return <div className="flex flex-col items-center justify-center py-24 gap-4 bg-white min-h-screen font-sans"><Loader2 className="animate-spin text-indigo-600" size={32} /><p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Menyiapkan Invoice...</p></div>;
  ...
  return (
    <div className="bg-[#F9FAFB] min-h-screen pb-24 font-sans relative">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
           <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <Subtext className="text-indigo-600 font-bold uppercase tracking-widest mt-0.5">{invoiceNumber}</Subtext>
              </div>
           </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
               <div className="relative" ref={clientDropdownRef}>
                  ...
                  <Button 
                    ...
                    className="w-full !px-4 !py-3 bg-white border !border-gray-200 font-medium text-sm text-left flex justify-between items-center group relative shadow-sm h-[46px]"
                    rightIcon={<ChevronDown className="text-gray-300" size={16} />}
                  >
                    <div className="absolute left-3.5 flex items-center justify-center h-full top-0">
                      <User className="text-gray-300" size={16} />
                    </div>
                    <span className="truncate ml-6">
                      {clientId ? clients.find(c => c.id.toString() === clientId)?.name : 'Pilih Pelanggan'}
                    </span>
                  </Button>
                  {isClientSearchOpen && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-xl shadow-2xl z-50 overflow-hidden">
                      <div className="p-3 border-b border-gray-50">
                         <Input 
                            ...
                            leftIcon={<Search className="text-gray-300" size={14} />}
         ...
         <Table>
            <TableHeader>
               ...
               <TableBody>
                  {items.map((item, idx) => (
                    <TableRow key={idx}>
                      <TableCell className="px-8 relative">
                        <div ref={idx === activeRowIdx ? productDropdownRef : null}>
                         <Button 
                            variant="ghost" 
                            onClick={() => { setActiveRowIdx(idx === activeRowIdx ? null : idx); setProductSearch(''); }} 
                            className="w-full !px-4 !py-2.5 bg-white border !border-gray-200 rounded text-xs text-left truncate flex items-center justify-between shadow-sm h-10"
                            rightIcon={<ChevronDown size={14} className="text-gray-300" />}
                         >
                            <span>{item.productId ? products.find(p => p.id.toString() === item.productId)?.name : 'Pilih Produk'}</span>
                         </Button>
                        </div>
                         {activeRowIdx === idx && (
                           <div className="absolute top-full left-8 right-8 bg-white border border-gray-100 shadow-xl z-50 rounded-lg overflow-hidden">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 space-y-5">
               <div className="flex items-center justify-between border-b border-gray-50 pb-4">
                  <Label className="uppercase tracking-widest">Subtotal</Label>
                  <span className="text-sm font-bold text-gray-900">{formatIDRVal(subtotal)}</span>
               </div>
               <div className="flex items-center justify-between border-b border-gray-50 pb-4">
                  <div className="flex items-center gap-4">
                     <Label className="uppercase tracking-widest">Diskon</Label>
                     <div className="flex border border-gray-200 rounded overflow-hidden h-9">
                        <select value={discountType} onChange={e => setDiscountType(e.target.value as any)} className="bg-gray-50 px-2 border-r text-[10px] font-bold uppercase outline-none focus:ring-0 active:ring-0">
               ...
               <div className="space-y-4 border-b border-gray-50 pb-4">
                  <div className="flex items-center justify-between">
                     <Label className="uppercase tracking-widest">Pajak</Label>
                     <div className="relative">
                        <Select 
                          value="" 
                          onChange={e => { if(e.target.value) { const id = Number(e.target.value); setSelectedTaxIds(prev => prev.includes(id) ? prev : [...prev, id]); } }} 
                          className="!px-4 !py-1.5 text-[10px] font-bold uppercase tracking-widest shadow-sm h-9"
                        >
               ...
               <div className="pt-2 pb-6 flex items-center justify-between">
                  <Label className="text-base uppercase tracking-widest">Grand Total</Label>
      ...
      >
        <div className="space-y-8 py-2">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Select label="Sapaan" value={quickClientForm.salutation} onChange={e => setQuickClientForm({...quickClientForm, salutation: e.target.value})}>
                 <option value="">Pilih</option>
                 <option value="Bapak">Bapak</option>
                 <option value="Ibu">Ibu</option>
              </Select>
              <div className="md:col-span-2">
                 <Input label="Nama Lengkap*" value={quickClientForm.name} onChange={e => setQuickClientForm({...quickClientForm, name: e.target.value})} placeholder="John Doe..." />
              </div>
           </div>
           <div className="space-y-3">
              <div className="flex items-center justify-between px-1">
                 <Label className="uppercase tracking-widest">Pilih Perusahaan Client</Label>
              ...
              {isAddingCo ? (
                 <div className="p-6 bg-blue-50/50 border border-blue-100 rounded-2xl space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                       <Input label="Nama Perusahaan*" placeholder="Nama PT..." value={newCo.name} onChange={e => setNewCo({...newCo, name: e.target.value})} className="bg-white border-blue-100" />
                       <Select label="Kategori*" value={newCo.category_id} onChange={e => setNewCo({...newCo, category_id: e.target.value})} className="bg-white border-blue-100">
                          <option value="">Pilih Kategori</option>
                          {clientCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                       </Select>
                    </div>
                    <Input label="Alamat Perusahaan*" placeholder="Alamat lengkap..." value={newCo.address} onChange={e => setNewCo({...newCo, address: e.target.value})} className="bg-white border-blue-100" />
                    <Button onClick={handleQuickAddCo} isLoading={coProcessing} className="w-full" leftIcon={<Save size={14} />}>
                      SIMPAN PERUSAHAAN
                    </Button>
                 </div>
              ) : (
                <Select value={quickClientForm.client_company_id} onChange={e => setQuickClientForm({...quickClientForm, client_company_id: e.target.value})}>
           ...
           <div className="grid grid-cols-2 gap-6">
              <Input label="Email" type="email" value={quickClientForm.email} onChange={e => setQuickClientForm({...quickClientForm, email: e.target.value})} placeholder="email@client.com" />
              <Input label="WhatsApp" value={quickClientForm.whatsapp} onChange={e => setQuickClientForm({...quickClientForm, whatsapp: e.target.value})} placeholder="08..." />
           </div>
        </div>
      </Modal>
      ...
      >
        <div className="space-y-8 py-2">
           <Input label="Nama Produk / Jasa*" value={quickProductForm.name} onChange={e => setQuickProductForm({...quickProductForm, name: e.target.value})} placeholder="Misal: Paket Langganan Pro..." />
           <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                 <div className="flex items-center justify-between px-1">
                    <Label className="uppercase tracking-widest">Kategori</Label>
                 ...
                 {isAddingCat ? (
                    <div className="flex gap-2 animate-in zoom-in-95">
                       <Input autoFocus placeholder="Kategori baru..." value={newCatName} onChange={e => setNewCatName(e.target.value)} className="flex-1 bg-white border-blue-200" />
                       <Button onClick={handleQuickAddCat} className="!px-3"><CheckIcon size={16} /></Button>
                    </div>
                 ) : (
                    <Select value={quickProductForm.category_id} onChange={e => setQuickProductForm({...quickProductForm, category_id: e.target.value})}>
                        <option value="">Pilih</option>
              ...
              <div className="space-y-2">
                 <div className="flex items-center justify-between px-1">
                    <Label className="uppercase tracking-widest">Satuan</Label>
                 ...
                 {isAddingUnit ? (
                    <div className="flex gap-2 animate-in zoom-in-95">
                       <Input autoFocus placeholder="Satuan baru..." value={newUnitName} onChange={e => setNewUnitName(e.target.value)} className="flex-1 bg-white border-blue-200" />
                       <Button onClick={handleQuickAddUnit} className="!px-3"><CheckIcon size={16} /></Button>
                    </div>
                 ) : (
                    <Select value={quickProductForm.unit_id} onChange={e => setQuickProductForm({...quickProductForm, unit_id: e.target.value})}>
                        <option value="">Pilih</option>
           ...
           <Input label="Harga Jual (IDR)*" type="number" value={quickProductForm.price} onChange={e => setQuickProductForm({...quickProductForm, price: Number(e.target.value)})} placeholder="0" />
           <Textarea label="Deskripsi" value={quickProductForm.description} onChange={e => setQuickProductForm({...quickProductForm, description: e.target.value})} className="h-32" placeholder="Detail produk..." />
        </div>
      </Modal>
