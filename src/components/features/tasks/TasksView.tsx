...
import { 
  ...
  Avatar, Table, TableHeader, TableRow, TableCell, 
  TableBody, TableEmpty, H1, H2, Subtext, Label, SearchInput
} from '@/components/ui';
...
export const TasksView: React.FC<Props> = ({ company, user, members, projectId }) => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-24">
      ...
      <Subtext className="!text-[10px] font-bold uppercase tracking-widest text-gray-400">Sinkronisasi Task Proyek...</Subtext>
    </div>
  );
      ...
      <div className="flex items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm shrink-0 overflow-x-auto custom-scrollbar">
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex-1">
             <H2 className="text-sm font-black text-gray-900 leading-none truncate uppercase tracking-tight">{project?.name}</H2>
             <Subtext className="!text-[9px] font-black text-emerald-600 uppercase tracking-widest mt-1">Daftar Pekerjaan Proyek</Subtext>
          </div>
        </div>
                ...
                <TableBody>
                  {filteredTasks.map(t => (
                    <TableRow key={t.id} className="hover:bg-gray-50/50 group transition-colors">
                      <TableCell>
                        <H2 className="text-sm font-bold text-gray-900 tracking-tight">{t.title}</H2>
                        {t.description && <Subtext className="text-[10px] text-gray-400 mt-0.5 line-clamp-1 italic">{t.description}</Subtext>}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                           <Avatar name={t.assigned_profile?.full_name} src={t.assigned_profile?.avatar_url} size="sm" className="bg-emerald-50 text-emerald-600 border border-emerald-100 font-bold" />
                           <Subtext className="text-[11px] font-bold text-gray-700">{t.assigned_profile?.full_name || 'Unassigned'}</Subtext>
                        </div>
                      </TableCell>
                      <TableCell>
                         <div className="space-y-1">
                            <Subtext className="text-[10px] font-bold text-gray-500 flex items-center gap-1.5"><Calendar size={10} /> {t.start_date ? new Date(t.start_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : '-'}</Subtext>
                            <Subtext className="text-[10px] font-bold text-gray-500 flex items-center gap-1.5"><Clock size={10} /> {t.end_date ? new Date(t.end_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }) : '-'}</Subtext>
                         </div>
                      </TableCell>
                      <TableCell>
                        <Badge variant="secondary" className="px-3 py-1 text-[9px] font-bold uppercase rounded-full">
              ...
              <div key={stage.id} className="flex flex-col gap-3 min-w-[300px] w-[300px] h-full">
                <div className={`p-4 ${STAGE_COLORS[sIdx % STAGE_COLORS.length]} rounded-2xl shadow-lg shadow-black/5 flex items-center justify-between border-b-4 border-black/10`}>
                   ...
                   <div className="flex items-center justify-center bg-white/20 px-2.5 py-1 rounded-lg border border-white/20">
                      <span className="text-[10px] font-black text-white">{tasksByStage[stage.id]?.length || 0}</span>
                   </div>
                </div>
                <div 
                     ...
                     <div 
                      ...
                      >
                        <div className="flex items-center justify-between mb-4">
                            <Badge variant="ghost" className="px-2 py-0.5 border border-gray-100 text-[8px] font-black text-gray-400 uppercase tracking-widest bg-gray-50">T-{String(t.id).padStart(3, '0')}</Badge>
                        ...
                        <H2 className="font-black text-[13px] text-gray-800 mb-3 leading-tight tracking-tight group-hover:text-emerald-600 transition-colors uppercase">{t.title}</H2>
                        <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                           <div className="flex items-center gap-2">
                              <Avatar name={t.assigned_profile?.full_name} src={t.assigned_profile?.avatar_url} size="sm" className="ring-2 ring-white shadow-sm font-black" />
                              <span className="text-[9px] font-black text-gray-400 uppercase tracking-tighter">{t.assigned_profile?.full_name?.split(' ')[0] || 'TBD'}</span>
                           </div>
                           {t.end_date && (
                             <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${new Date(t.end_date) < new Date() ? 'bg-rose-50 text-rose-500' : 'bg-gray-50 text-gray-400'}`}>
                                <Calendar size={10} />
