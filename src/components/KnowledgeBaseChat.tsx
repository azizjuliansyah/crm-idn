    ...
    <div className="space-y-3 leading-relaxed">
      {lines.map((line, idx) => {
        ...
        if (trimmed.startsWith('* ') || trimmed.startsWith('- ')) {
          return (
            <div key={idx} className="flex gap-3 pl-2 py-0.5 animate-in slide-in-from-left-1 duration-300">
...
export const KnowledgeBaseChat: React.FC<Props> = ({ 
  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="fixed bottom-8 right-8 z-[60] w-16 h-16 bg-indigo-600 text-white rounded-2xl flex items-center justify-center shadow-2xl hover:scale-105 active:scale-95 transition-all animate-in zoom-in-50 duration-300 group"
      >
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[60] w-[440px] bg-white border border-gray-100 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col h-[600px] animate-in slide-in-from-bottom-10 fade-in duration-300">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {msg.references.map(ref => (
                      <button 
                        ...
                        className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-gray-200 text-[10px] font-bold text-gray-600 hover:border-indigo-500 hover:text-indigo-600 hover:shadow-md transition-all animate-in zoom-in duration-300"
                      >
      ...
      <div className="p-6 bg-white border-t border-gray-50 shrink-0">
          {!isApiKeyActive ? (
              <div className="bg-amber-50 p-5 rounded-3xl border border-amber-100 space-y-4 animate-in zoom-in-95 duration-300">
