...
export const Modal: React.FC<ModalProps> = ({ 
  ...
  return (
    ...
    >
      <div 
        ref={modalRef}
        className={`w-full ${sizeClasses[size]} bg-white rounded-2xl shadow-2xl shadow-gray-900/10 overflow-hidden transform animate-in zoom-in-95 slide-in-from-bottom-4 duration-300`}
      >
        {!hideClose && (
          <div className={`px-8 pt-8 pb-4 flex items-center justify-between border-b border-gray-50 ${title === '' ? 'absolute top-0 right-0 z-50 border-none bg-transparent' : ''}`}>
            ...
            <button 
              onClick={onClose}
              className={`p-2 rounded-full transition-all ${title === '' ? 'text-gray-400 hover:text-gray-900 bg-white/50 backdrop-blur-sm' : 'text-gray-400 hover:text-gray-900 hover:bg-gray-50'}`}
            >
              <X size={20} />
            </button>
          </div>
