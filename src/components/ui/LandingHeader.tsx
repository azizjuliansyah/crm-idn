    ...
    <header className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 h-20 flex items-center">
      <div className="container mx-auto px-6 lg:px-12 flex items-center justify-between">
        <div className="flex items-center cursor-pointer" onClick={() => onNavigate && onNavigate('/')}>
            ...
            <span className="text-xl font-bold tracking-tighter text-gray-900">CRM<span className="text-blue-600">IDN</span></span>
        </div>
        
        <nav className="hidden md:flex items-center gap-8">
            {['Fitur', 'Solusi', 'Harga', 'Kontak'].map((item) => (
               <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-bold text-gray-500 hover:text-blue-600 transition-colors uppercase tracking-wide">
                  {item}
               </a>
            ))}
        </nav>
        ...
        <div className="flex items-center gap-4">
          {session ? (
            <Link 
              href="/dashboard"
              className="px-6 py-3 bg-gray-900 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-black transition-all shadow-lg flex items-center gap-2"
            >
            ...
            <>
              <Link 
                href="/login"
                className="hidden md:flex px-6 py-3 text-gray-600 font-bold text-xs uppercase tracking-widest hover:text-blue-600 transition-colors"
               >
                 Masuk
               </Link>
              <Link 
