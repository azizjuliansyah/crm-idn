...
const FlowShape: React.FC<{ 
  ...
  const NumberBadge = displayNumber !== null ? (
    <div className="absolute -top-2 -right-2 w-5 h-5 rounded-full border border-gray-900 bg-white flex items-center justify-center z-20 shadow-sm">
      <span className="text-[8px] font-bold text-gray-900">{displayNumber}</span>
    </div>
  ) : null;
  ...
  if (type === 'start' || type === 'end') {
    return (
      <div className="relative flex flex-col items-center animate-in fade-in zoom-in-95 duration-300">
        {NumberBadge}
        <div className="w-24 h-10 border border-gray-900 rounded-full flex items-center justify-center text-[8px] font-bold uppercase tracking-widest bg-white px-2 text-center shadow-sm">
          {displayLabel}
        </div>
      </div>
    );
    ...
    return (
      <div className="relative flex flex-col items-center animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className={`${sideClass} border border-gray-900 rotate-45 flex items-center justify-center bg-white shadow-sm`}>
          <div className={`-rotate-45 ${textClass} font-bold uppercase text-center px-2 leading-tight break-words max-w-[85%]`}>
            {displayLabel}
          </div>
        </div>
      </div>
  ...
  if (isConnector) {
    return (
      <div 
        onClick={onClick}
        className={`relative flex flex-col items-center animate-in fade-in zoom-in-95 duration-300 ${onClick ? 'cursor-pointer group/conn' : ''}`}
      >
        <div className={`w-8 h-8 border border-gray-900 rounded-full flex items-center justify-center text-[9px] font-bold bg-white shadow-sm uppercase px-1 text-center transition-all ${onClick ? 'group-hover/conn:bg-blue-600 group-hover/conn:text-white group-hover/conn:scale-110 group-active/conn:scale-95' : ''}`}>
          {displayLabel}
        </div>
      </div>
    );
  ...
  return (
    <div className="relative flex flex-col items-center animate-in fade-in zoom-in-95 duration-300">
      {NumberBadge}
      <div className="w-28 h-12 border border-gray-900 flex items-center justify-center text-[8px] font-bold uppercase text-center bg-white shadow-sm px-2 leading-tight">
        {displayLabel}
      </div>
    </div>
  );
    ...
    <div className="bg-[#F9FAFB] min-h-screen">
       <div className="sticky top-0 z-40 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-5">
             ...
             <div>
                <h1 className="text-xl font-bold text-gray-900 tracking-tight">{sop.title}</h1>
                <div className="flex items-center gap-3 mt-0.5">
                   <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">{sop.document_number}</p>
                   <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                   <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Rev {String(sop.revision_number).padStart(2, '0')}</p>
                </div>
             </div>
          </div>
          <div className="flex gap-3">
             <button onClick={handleExportPDF} className="px-5 py-2.5 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-600 hover:bg-gray-50 flex items-center gap-2"><Printer size={16} /> PDF Export</button>
             {sop.status === 'Approved' ? (
                <button onClick={handleRevision} disabled={isProcessing} className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-indigo-100 active:scale-95">
                  {isProcessing ? <Loader2 className="animate-spin" size={16} /> : <RefreshCw size={16} />} Revisi SOP
                </button>
             ) : (
                <button onClick={() => router.push(`/dashboard/sops/${sop.id}/edit`)} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-blue-100 active:scale-95">
                   <Edit size={16} /> Lanjut Edit
                </button>
             )}
          </div>
       </div>
       ...
       <div className="max-w-[98%] mx-auto px-6 py-10 space-y-10">
             ...
             {[
               ...
               <div key={sec.id} className="space-y-3">
                  <div className="flex items-center gap-3">
                     ...
                     <h4 className="text-sm font-bold text-gray-900 uppercase tracking-widest">{sec.label}</h4>
                  </div>
                  <p className="text-sm text-gray-600 font-medium leading-relaxed pl-11 whitespace-pre-wrap">{ (sop as any)[sec.id] || '-' }</p>
               </div>
             ))}
          </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
             <div className="p-8 border-b border-gray-50 bg-gray-50/20">
                <h4 className="text-sm font-bold text-gray-900 uppercase tracking-widest">Visualisasi Alur & Instruksi Kerja</h4>
             </div>
             <table className="w-full border-collapse table-fixed">
                <thead>
                   <tr className="bg-gray-50/50">
                      <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center w-[340px]">Diagram Alir</th>
                      <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-left w-48">Pelaksana</th>
                      <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-left min-w-[400px]">Instruksi Pekerjaan</th>
                      <th className="px-8 py-5 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-left w-48">Dokumen</th>
                   </tr>
                </thead>
                <tbody>
                   <tr className="border-b border-gray-50">
                                 ...
                                 <div key={flowIdx} data-flow-label={flowStep.step_name} className="flex flex-col items-center w-full group/step">
                                    <div className="relative flex items-center justify-center w-full">
                                      ...
                                      {sideYesTarget && (
                                        <div className="absolute top-1/2 -translate-y-1/2 flex items-center" style={{ left: `calc(50% + ${currentEdgeOffset}px)` }}>
                                          <div className="w-12 h-0.5 bg-gray-900 relative">
                                            <div className="absolute -top-4 left-1 text-[8px] font-bold text-emerald-600 bg-white px-1 border border-emerald-100 rounded">YA</div>
                                      ...
                                      {sideNoTarget && (
                                        <div className="absolute top-1/2 -translate-y-1/2 flex items-center" style={{ left: `calc(50% + ${currentEdgeOffset}px)` }}>
                                          <div className="w-12 h-0.5 bg-gray-900 relative">
                                            <div className="absolute -top-4 left-1 text-[8px] font-bold text-rose-600 bg-white px-1 border border-rose-100 rounded">TIDAK</div>
                                            <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-0.5">
                                              <ArrowRight size={14} strokeWidth={4} className="text-gray-900" />
                                            </div>
                                          </div>
                                      ...
                                      <div className={`flex flex-col items-center relative w-full ${flowsToAlurBaru ? 'h-24' : 'h-16'}`}>
                                         {shouldDrawVerticalArrow ? (
                                           <React.Fragment>
                                             ...
                                             >
                                                {verticalLabel && (
                                                  <div className={`absolute left-2 top-2 text-[8px] font-bold px-1 bg-white border rounded ${verticalLabel === 'YA' ? 'text-emerald-600 border-emerald-100' : 'text-rose-600 border-rose-100'}`}>
                                                     {verticalLabel}
                                                  </div>
                                                )}
                      ...
                      <td colSpan={3} className="px-8 py-10 bg-white"></td>
                   </tr>
                   ...
                   {contentSteps.map((step, idx) => {
                      ...
                      return (
                        <tr key={idx} className={`group transition-colors border-b border-gray-50 ${isEnd ? 'bg-gray-50/10' : 'hover:bg-gray-50/5'}`}>
                          <td className="px-8 py-10 align-top">
                             {(!isEnd && !isAlurBaru) && step.responsible_role && (
                               <div className="flex items-center gap-2 mt-2">
                                 <User size={12} className="text-blue-500" />
                                 <span className="text-xs font-bold text-gray-900 uppercase tracking-tight leading-tight">{step.responsible_role}</span>
                               </div>
                             )}
                          </td>
                          ...
                          <td className="px-8 py-10 align-top">
                             <div className="space-y-4 mt-2">
                               <div className="flex gap-4">
                                   {!isEnd && !isAlurBaru && (
                                      <span className="text-xs font-bold text-gray-400 bg-gray-50 w-6 h-6 rounded flex items-center justify-center shrink-0">{stepDisplayNumbers[idx]}</span>
                                   )}
                                   <div className="space-y-4 flex-1">
                                     {!isEnd && !isAlurBaru && (
                                       <p className="text-sm text-gray-700 leading-relaxed font-medium whitespace-pre-wrap">{step.description}</p>
                                     )}
                                   </div>
                               </div>
                             </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 overflow-hidden">
             <h4 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-8 text-center">Lembar Otorisasi</h4>
             <div className="grid grid-cols-3 divide-x divide-gray-100 border border-gray-100 rounded-2xl overflow-hidden">
                <div className="p-6 text-center">
                   <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-10">Disiapkan Oleh</p>
                   <p className="text-xs font-bold text-gray-900">{sop.prepared_by || '-'}</p>
                </div>
                <div className="p-6 text-center">
                   <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-10">Diperiksa Oleh</p>
                   <p className="text-xs font-bold text-gray-900">{sop.checked_by || '-'}</p>
                </div>
                <div className="p-6 text-center">
                   <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-10">Disahkan Oleh</p>
                   <p className="text-xs font-bold text-gray-900">{sop.approved_by || '-'}</p>
                </div>
             </div>
          </div>
       </div>
