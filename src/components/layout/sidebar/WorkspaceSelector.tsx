    ...
    <div className="p-4" ref={dropdownRef}>
      <div className="relative">
        <Button
          ...
          variant="ghost-dark"
          align="left" size='sm'
          className={`w-full group flex items-center gap-3 p-3 rounded-2xl transition-all duration-300 border cursor-pointer !normal-case ! ${isDropdownOpen ? 'bg-gray-900 border-gray-700 shadow-xl shadow-black/20 ring-4 ring-gray-900/50' : 'bg-gray-900/40 border-gray-800 hover:border-gray-700 hover:bg-white/5'}`}
        >
          <div className="relative">
        ...
        {isDropdownOpen && (
          <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl z-50 py-3 overflow-hidden">
            <div className="max-h-[60vh] flex flex-col">
              {isAdmin && (
                <div className="px-2 pb-2 mb-2 border-b border-gray-800">
                  <Button onClick={() => { switchCompany(null); setIsDropdownOpen(false); }} variant="ghost-dark" align="left" className={`w-full !px-3 !py-2 flex items-center gap-3 rounded-xl transition-all cursor-pointer !normal-case !h-auto ${!activeCompany ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'hover:bg-white/5 text-gray-300'}`}>
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${!activeCompany ? 'bg-white text-blue-600' : 'bg-gray-800 text-gray-500'}`}><Monitor size={14} /></div>
                    <Label className={`text-[11px] font-semibold !capitalize ! cursor-pointer ${!activeCompany ? 'text-white' : 'text-gray-300'}`}>Platform Central</Label>
                  </Button>
                </div>
              )}
              <div className="px-3 pb-2">
                <div className="relative group/search">
                  ...
                  <Input 
                    ...
                    className="!w-full !pl-9 !pr-3 !py-2 !bg-gray-950 !border-gray-800 focus:!border-blue-500/50 focus:!bg-black !rounded-xl !text-[11px] !font-medium !outline-none !transition-all !text-white !h-auto placeholder:text-gray-600" 
                  />
                </div>
              </div>
              <div className="px-2 space-y-1 overflow-y-auto custom-scrollbar">
                {filteredCompanies.map(co => (
                  <Button key={co.id} onClick={() => { switchCompany(co); setIsDropdownOpen(false); }} variant="ghost-dark" align="left" className={`w-full !px-3 !py-2 flex items-center gap-3 rounded-xl transition-all !capitalize !normal-case !h-auto cursor-pointer ${activeCompany?.id === co.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'hover:bg-white/5 text-gray-300'}`}>
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${activeCompany?.id === co.id ? 'bg-white text-indigo-600' : 'bg-gray-800 text-gray-500'}`}>{co.logo_url ? <img src={co.logo_url} className="w-full h-full object-cover rounded-lg" alt="Logo" /> : co.name.charAt(0)}</div>
                    <Label className={`text-[11px] truncate !capitalize ! cursor-pointer font-medium ${activeCompany?.id === co.id ? 'text-white' : 'text-gray-300'}`}>{co.name}</Label>
                  </Button>
                ))}
              </div>
              <div className="mt-2 pt-2 border-t border-gray-800 px-2 space-y-1">
                <Link href={getPathFromViewId('profil_saya')} onClick={() => setIsDropdownOpen(false)} className="w-full px-3 py-2 flex items-center gap-3 rounded-xl text-gray-400 hover:bg-white/5 transition-all cursor-pointer group">
                  <div className="w-8 h-8 rounded-lg bg-gray-800 flex items-center justify-center text-gray-600 group-hover:text-gray-300 transition-colors"><User size={14} /></div>
