    ...
    <div className="p-4" ref={dropdownRef}>
      <div className="relative">
        <Button
          ...
          variant="ghost-dark"
          align="left" size='sm'
          className={`w-full group flex items-center gap-3 p-3 rounded-2xl transition-all duration-300 border cursor-pointer !normal-case ! ${isDropdownOpen ? 'bg-slate-800 border-slate-700 shadow-xl shadow-black/20 ring-4 ring-slate-800/50' : 'bg-slate-800/40 border-slate-800 hover:border-slate-700 hover:bg-white/5'}`}
        >
          <div className="relative">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white font-medium text-xs shadow-lg transition-transform group-hover:scale-105 duration-300 overflow-hidden ${(!activeCompany && isAdmin) ? 'bg-blue-600' : 'bg-slate-800'}`}>
        ...
        {isDropdownOpen && (
          <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl z-50 py-3 overflow-hidden">
            <div className="max-h-[60vh] flex flex-col">
              {isAdmin && (
                <div className="px-2 pb-2 mb-2 border-b border-slate-800">
                  <Button onClick={() => { switchCompany(null); setIsDropdownOpen(false); }} variant="ghost-dark" align="left" className={`w-full !px-3 !py-2 flex items-center gap-3 rounded-xl transition-all cursor-pointer !normal-case !h-auto ${!activeCompany ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'hover:bg-white/5 text-slate-300'}`}>
