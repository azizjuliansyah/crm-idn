...
export const generateTemplate6 = async (
  ...
  safeRect(doc, col1 - 8, iconY + 2, 8, 5, 'FD');
  doc.setDrawColor(255, 255, 255);
  doc.setLineWidth(0.5);
  doc.line(col1 - 8, iconY + 2, col1 - 4, iconY + 4.5);
  doc.line(col1, iconY + 2, col1 - 4, iconY + 4.5);

  safeText(doc, config.company_address || 'info@idn.id\nJl. Anggrek Rosliana no 12A Slipi\nPalmerah Jakarta Barat 11480', col1 - 8, contactY);

  // Border between 1 and 2
  doc.setDrawColor('#E5E7EB');
  doc.setLineWidth(0.3);
  doc.line(col2 - 18, iconY, col2 - 18, contactY + 8);
  ...
  doc.addImage(phoneBase64, 'PNG', col2 - 12, iconY + 1, 6, 6);

  safeText(doc, config.contact_phone_hours || '0819-0819-1001\nSenin - Jumat\n09.00 to 17.00 WIB', col2 - 12, contactY);
  ...
  safeText(doc, config.company_website || 'www.idn.id', col3 + 3, contactY - 0.5, { align: 'center' });

  // Bottom Border separator for header
  doc.setDrawColor('#E5E7EB');
  doc.setLineWidth(0.5);
  doc.line(82, contactY + 12, pageWidth - padX, contactY + 12);
  ...
  // Baris UP
  let currentClientY = clientStartY + 14;
  doc.setFont('helvetica', 'bold');
  safeText(doc, 'UP: ', padX, currentClientY);
  let labelW = (doc.getStringUnitWidth('UP: ') * doc.getFontSize()) / doc.internal.scaleFactor;
  doc.setFont('helvetica', 'normal');
  safeText(doc, `${qData.client?.salutation || ''} ${qData.client?.name || ''}`.trim(), padX + labelW, currentClientY);

  // Baris Address
  if (qData.client?.address) {
    currentClientY += 5;
    safeText(doc, String(qData.client?.address || ''), padX, currentClientY, { maxWidth: 80 });
    currentClientY += 5; // Extra gap if address exists
  }

  currentClientY += 5; // Standard gap before contact info

  // Baris Phone
  doc.setFont('helvetica', 'bold');
  safeText(doc, 'Phone: ', padX, currentClientY);
  labelW = (doc.getStringUnitWidth('Phone: ') * doc.getFontSize()) / doc.internal.scaleFactor;
  doc.setFont('helvetica', 'normal');
  safeText(doc, String(qData.client?.whatsapp || ''), padX + labelW, currentClientY);

  // Baris Email
  currentClientY += 5;
  doc.setFont('helvetica', 'bold');
  safeText(doc, 'Email: ', padX, currentClientY);
  labelW = (doc.getStringUnitWidth('Email: ') * doc.getFontSize()) / doc.internal.scaleFactor;
  doc.setFont('helvetica', 'normal');
  safeText(doc, String(qData.client?.email || ''), padX + labelW, currentClientY);

  // Cyan Banner Background
  const bannerY = 85;
  const bannerW = 110 + padX; 
  doc.setFillColor(cyanColor);
  ...
  const commonOptions: any = {
    ...
    margin: { left: 0, right: 0, bottom: 30 },
    didParseCell: (data: any) => {
      data.cell.styles.halign = 'left';
      data.cell.styles.cellPadding = { top: 3, bottom: 3, left: 2, right: 2 };
      if (data.column.index === 0) {
        data.cell.styles.cellPadding.left = padX;
      }
      if (data.column.index === 3) {
        data.cell.styles.cellPadding.right = padX;
      }
    },
    ...
    didDrawCell: (data: any) => {
      if (data.section === 'body' && data.column.index === 0) {
        ...
        if (lines && Array.isArray(lines) && lines.length > 0) {
          ...
          const sX = data.cell.x + (data.cell.styles.cellPadding?.left || padX);
          if (productName) {
            doc.setFont('helvetica', 'bold');
            doc.setTextColor(textGray);
  ...
  safeRect(doc, 0, finalY + 5, (pageWidth / 2), 7, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(9); doc.setFont('helvetica', 'bold');
  safeText(doc, `${dateTitle} `, padX, finalY + 9.5);

  const dateTitleWidth = (doc.getStringUnitWidth(`${dateTitle} `) * doc.getFontSize()) / doc.internal.scaleFactor;
  doc.setFont('helvetica', 'bold');
  safeText(doc, formatDateString(printDate), padX + dateTitleWidth, finalY + 9.5);

  const rightLabelX = pageWidth - 60 - padX;
  const xCol2 = 95 + 35; // Start of Jumlah column
  const xCol3 = xCol2 + 20; // Start of Total column
  const summaryLabelX = xCol2 + 2;
  const summaryValueX = xCol3 + 2;

  doc.setTextColor(textGray);
  doc.setFontSize(10); doc.setFont('helvetica', 'normal');
  safeText(doc, 'Sub Total', summaryLabelX, finalY + 10);
  safeText(doc, formatIDR(qData.subtotal), summaryValueX, finalY + 10);
  ...
  safeText(doc, taxLabel, summaryLabelX, currentY);
  safeText(doc, formatIDR(qData.tax_value || 0), summaryValueX, currentY);

  const grandTotalY = currentY + 3;
  doc.setFillColor(cyanColor);
  safeRect(doc, xCol2, grandTotalY, pageWidth - xCol2, 8, 'F');
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  safeText(doc, 'Total', summaryLabelX, grandTotalY + 5.5);
  safeText(doc, formatIDR(qData.total), summaryValueX, grandTotalY + 5.5);
